...
function TicketImage({ src, alt, className = "", preload = false }: { src: string; alt: string; className?: string; preload?: boolean }) {
  return (
    <div className={`figma-passport-ticket ${className}`.trim()}>
      <Image className="figma-ticket-composite" src={src} alt={alt} fill sizes="(max-width: 402px) calc(100vw - 30px), 372px" preload={preload} />
    </div>
  );
}
...
function Welcome({ connected, onStart, onQr }: { connected: boolean; onStart: () => void; onQr: () => void }) {
  return (
    <div className="screen welcome-screen">
      ...
      <TicketImage src="/images/passport-ticket.svg" alt="Style Journey Passport · Guest Jiyoon · Destination Curated by You" className="welcome-passport-ticket" preload />
...
function PassportOffer({ onIssue, onBack }: { onIssue: () => void; onBack: () => void }) {
  return (
    <div className="screen passport-offer-screen">
      ...
      <TicketImage src="/images/passport-ticket.svg" alt="Style Journey Passport · Guest Jiyoon · Destination Curated by You" className="passport-offer-card" preload />
...
function Boarding({ stampCount, onBack, onContinue }: { stampCount: number; onBack: () => void; onContinue: () => void }) {
  return (
    <div className="screen boarding-screen">
      ...
      <TicketImage src="/images/boarding-pass-ready.svg" alt="Private Boarding Pass · Gate S1 · Guest Jiyoon · Ready to Board" className="figma-boarding-pass" preload />
